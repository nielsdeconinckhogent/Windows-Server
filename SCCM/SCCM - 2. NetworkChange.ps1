...
.DESCRIPTION
  Dit script zal de netwerkadapter veranderen naar Intern. Intern zal bijgevolg het IP adres 192.168.100.40 toegewezen krijgen.
.NOTES
  Version:        1.0
...
function SetIPAndDns {
    ...
    New-NetIPAddress -InterfaceIndex $adapter.ifIndex -IPAddress 192.168.100.40 -PrefixLength 24 -DefaultGateway 192.168.100.10 -ErrorAction Stop
...
#-----------------------------------------------------------[Execution]------------------------------------------------------------
while($go -eq 1) {
Write-Host -BackgroundColor Black -ForegroundColor Cyan "Dit script zal de netwerkadapters van $env:computername veranderen Intern en het ip adres van Intern veranderen."
